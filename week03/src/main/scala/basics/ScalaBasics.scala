...
object ScalaBasics {
   ...
   */
  def oddRange(n: Int): Range = 1 to (n * 2) by 2
  ...
  def minRecursive(r: Array[Int]): Int = {
    ...
    if (r.length == 1) r(0)
    else scala.math.min(r(0), minRecursive(r.slice(1,r.length)))
   ...
   */
  def splitInHalf(s: String): (String, String) = (s.slice(0,s.length/2), s.slice(s.length/2, s.length))
